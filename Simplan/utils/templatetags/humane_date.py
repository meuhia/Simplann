...
def do_humane_date(parser, token):
    ...
    if not len(content) in (2, 3):
        raise SyntaxError, "Usage : {% humane_date variable (optional user timezone) %}"
...
class HumaneDateNode(template.Node):
    ...
    def render(self, context):
        ...
        date_nn = timezone(Simplan.settings.TIME_ZONE).localize(date)
        date_tz = date_nn.astimezone(tz)
        return humane_date(date_tz)


def set_timezone(request):
    tz = timezone(Simplan.settings.TIME_ZONE)
